...
const siteSlug = computed(() => String(params.siteSlug || 'pas slug'))
const articleId = computed(() => Number(params.articleId || 0))
const articleSlug = computed(() => String(params.articleSlug || 'no slug'))
...
<template>
  <section>
    ...
    <div v-else>
      <h3 class="text-2xl font-bold">{{ article.post_title ?? 'Article has no title' }}</h3>
